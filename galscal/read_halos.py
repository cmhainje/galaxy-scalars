...
class DarkHalo:
    ...
    def load_positions_and_velocities(self, shift=True, pos_center=None):
        halo_dark_dm = il.snapshot.loadHalo(
            self.base_path, self.snap_num, self.idx_halo_dark, "dm"
        )
    ...
    def shift_points_torus(self, points, shift):
        return (
            points - shift + 0.5 * self.box_size
    ...
    def compute_m200m_fof(self, r200m, pos_center, m_dmpart_dark):
        x_data_halo, v_data_halo = self.load_positions_and_velocities(
            shift=True, pos_center=pos_center
        )
    ...
    def compute_mrv_200m_fof(
        self, density_mean, m_dmpart_dark, log_mass_shift, pos_center, r_max=None
    ):
    ...
    def compute_MXV_rms(self, center_mode, m_dmpart_dark):
        x_data_halo, v_data_halo = self.load_positions_and_velocities(
            shift=True, center_mode=center_mode
        )
        dists_from_center = np.linalg.norm(x_data_halo, axis=1)
    ...
    def get_a_mfrac(self, mfrac):
        if (
            "MAH" not in self.catalog_properties
            or len(self.catalog_properties["MAH"][0]) == 0
        ):
            return 1.0  # ??
        ...
        i_a1 = np.where(a_vals == 1.0)[0][
            0
        ]  # there should be exactly 1, but if not, eh take first
        m_a1 = m_vals[i_a1]
        a_mfrac_interp = utils.y_interpolated(m_vals / m_a1, a_vals, mfrac)
        if (
            a_mfrac_interp < 0 or a_mfrac_interp > 1 or np.isnan(a_mfrac_interp)
...
class SimulationReader:
    ...
    def __init__(self, base_dir, sim_name, sim_name_dark, snap_num_str):
        ...
        with h5py.File(
            f"{self.base_path_dark}/snapdir_{self.snap_num_str}/snap_{self.snap_num_str}.0.hdf5",
            "r",
        ) as f:
        ...
        with h5py.File(
            f"{self.base_path_hydro}/snapdir_{self.snap_num_str}/snap_{self.snap_num_str}.0.hdf5",
            "r",
        ) as f:
            header = dict(f["Header"].attrs.items())
    ...
    def read_simulations(self, subhalo_fields_to_load=None):
        ...
        self.subhalos_hydro = il.groupcat.loadSubhalos(
            self.base_path_hydro, self.snap_num, fields=subhalo_fields_to_load
        )
        self.halos_hydro = il.groupcat.loadHalos(self.base_path_hydro, self.snap_num)

        self.subhalos_dark = il.groupcat.loadSubhalos(
            self.base_path_dark, self.snap_num
        )
    ...
    def match_twins(self):
        ...
        if os.path.exists(fn_match_dark_to_full) and os.path.exists(
            fn_match_full_to_dark
        ):
            self.subhalo_dark_to_full_dict = np.load(
                fn_match_dark_to_full, allow_pickle=True
            ).item()
            self.subhalo_full_to_dark_dict = np.load(
    ...
    def get_halos_with_SAM_match(self, idxs_halo_dark):
        ...
        matches = True
        halos_sam = ilsam.groupcat.load_snapshot_halos(
            self.base_path_sam, self.snap_num, subvolume_list, fields, matches
        )
        idxs_halo_dark_SAM = halos_sam["HalopropFoFIndex_DM"]
        i_with_SAM_match = np.in1d(idxs_halo_dark, idxs_halo_dark_SAM)
        print(
            f"{np.sum(i_with_SAM_match)}/{len(i_with_SAM_match)} halos have with SAM matches"
        )
    ...
    def get_halos_with_mah_info(self, halo_tag):
        ...
        print(
            f"{np.sum(i_with_mah_info)}/{len(i_with_mah_info)} halos have with mah info"
        )
    ...
    def construct_halo_table(self, fn_halos, overwrite=False, N=None):
        ...
        self.halos_dark["GroupFirstSub"] = self.halos_dark["GroupFirstSub"].astype(
            "int32"
        )
        ...
        i_has_hydro_match = np.isin(
            self.halos_dark["GroupFirstSub"],
            list(self.subhalo_dark_to_full_dict.keys()),
        )
        ...
        idxs_subtwins_hydro = [
            self.subhalo_dark_to_full_dict[idx] for idx in idxs_subhalos_dark
        ]
        # This is that hydro subhalo's parent halo in the hydro sim
        idxs_halos_hydro = [
            self.subhalos_hydro["SubhaloGrNr"][idx] for idx in idxs_subtwins_hydro
        ]
    ...
    def compute_velocity(self, mass_msunperh, radius_ckpcperh):
        ...
        # make sure to include!
        G = const.G.to("(kpc * km**2)/(Msun * s**2)")
        velocity_kpcpers = np.sqrt(
            G * (mass_msunperh * u.Msun) / (radius_ckpcperh * u.kpc)
        )
    ...
    def add_properties_dark(self, fn_halos, overwrite=True):
        ...
        tab_halos["v200m"] = self.compute_velocity(
            self.mass_multiplier * self.halos_dark["Group_M_Mean200"][idxs_halos_dark],
            self.halos_dark["Group_R_Mean200"][idxs_halos_dark],
        )
        ...
        ### velocity dispersion
        tab_halos["veldisp_subhalo"] = self.subhalos_dark["SubhaloVelDisp"][
            idxs_subhalos_dark
        ]
    ...
    def add_properties_hydro(self, fn_halos, overwrite=True):
        ...
        tab_halos["mstellar"] = self.subhalos_hydro["SubhaloMassType"][
            :, self.ipart_star
        ][idxs_subhalos_hydro]
        tab_halos["rstellar"] = self.subhalos_hydro["SubhaloHalfmassRadType"][
            :, self.ipart_star
        ][idxs_subhalos_hydro]
        tab_halos["mgas"] = self.subhalos_hydro["SubhaloMassType"][:, self.ipart_gas][
            idxs_subhalos_hydro
        ]
        ...
        tab_halos["mbh_per_mstellar"] = (
            self.subhalos_hydro["SubhaloBHMass"][idxs_subhalos_hydro]
            / self.subhalos_hydro["SubhaloMassType"][:, self.ipart_star][
                idxs_subhalos_hydro
            ]
        )
        tab_halos["npartstellar"] = self.subhalos_hydro["SubhaloLenType"][
            :, self.ipart_star
        ][idxs_subhalos_hydro]
        tab_halos["npartgas"] = self.subhalos_hydro["SubhaloLenType"][
            :, self.ipart_gas
        ][idxs_subhalos_hydro]
        tab_halos["x_minPE_hydro"] = self.subhalos_hydro["SubhaloPos"][
            idxs_subhalos_hydro
        ]
    ...
    def add_mv200m_fof_dark(self, fn_halos, overwrite=True):
        ...
        for i in i_shuffle:
            ...
            halo = DarkHalo(
                idx_halo_dark, self.base_path_dark, self.snap_num, self.box_size
            )
            # units of 10^10 msun bc that's what m_dmpart_dark is in
            m200m_fof[i] = halo.compute_m200m_fof(
                tab_halos["r200m"][i], tab_halos["x_minPE"][i], self.m_dmpart_dark
            )
            if count % 1000 == 0:
        ...
        tab_halos["v200m_fof"] = self.compute_velocity(
            self.mass_multiplier * m200m_fof, tab_halos["r200m"]
        )
    ...
    def add_properties_structure(self, fn_halos, overwrite=True):
        ...
        with h5py.File(
            f"{self.tng_path_dark}/postprocessing/halo_structure_{self.snap_num_str}.hdf5",
            "r",
        ) as f:
    ...
    def transform_properties(self, fn_halos, overwrite=True):
        ...
        names_mass = ["m200m", "m200m_fof", "m200m_hydro", "mstellar", "mgas"]
        ...
        for name_sfr in names_sfr:
            ...
            tab_halos["log_s" + name_sfr] = self.log_sfr_to_log_ssfr(
                np.log10(sfr), tab_halos["mstellar"] * self.mass_multiplier
            )
        ...
        tab_halos["log_mbh_per_mstellar"] = (
            tab_halos["log_mbh"] - tab_halos["log_mstellar"]
        )
    ...
    def write_MAH_table(self, fn_halos, fn_mahs, overwrite=False):
        ...
        halo_idx_to_root_idx_dict = dict(
            zip(halos_sam["HalopropFoFIndex_DM"], halos_sam["HalopropRootHaloID"])
        )
        ...
        for root_idx in halos_sam["HalopropRootHaloID"]:
            mtree = ilsam.merger.load_tree_haloprop(
                self.base_path_sam,
                root_idx,
                fields=["HalopropRedshift", "HalopropMvir", "HalopropSnapNum"],
                most_massive=True,
                matches=True,
            )
            ...
            if n_snaps == len(snap_nums_str_all):
                ...
                snap_to_a_dict = dict(zip(snap_nums_str, scale_factors))
        ...
        for i, idx_halo_dark in enumerate(idxs_halos_dark):
            ...
            try:
                mtree = ilsam.merger.load_tree_haloprop(
                    self.base_path_sam,
                    root_idx,
                    fields=["HalopropMvir", "HalopropSnapNum"],
                    most_massive=True,
                    matches=True,
                )
    ...
    def write_amfrac_table(self, fn_mahs, fn_amfrac, overwrite=True, mfrac_width=0.025):
    ...
    def select_halos(
        ...
        halo_tag,
        fn_select,
        num_star_particles_min=0,
        num_gas_particles_min=0,
        halo_logmass_min=None,
        ...
        if halo_mass_difference_factor is not None:
            ...
            i_mdiff_abovemin = (
                (mhalo_dark / mhalo_hydro < halo_mass_difference_factor)
        ...
        if must_have_halo_structure_info:
            i_with_halo_structure_info = self.get_halos_with_structure_info(
                tab_halos["idx_halo_dark"]
            )
        ...
        print(len(random_ints))
        tab_select = Table(
            [tab_halos["idx_halo_dark"][i_select], idxs_table[i_select], random_ints],
    ...
    def add_catalog_property_to_halos(self, property_name, halo_tag=None):
        if property_name == "sfr_hydro_subhalo_1Gyr":
            with h5py.File(
                ...
                for halo in self.dark_halo_arr:
                    halo.set_catalog_property(
                        property_name, idx_subhalo_to_sfr1[halo.idx_subhalo_hydro]
                    )
            return
        ...
        if (
            property_name == "m200mean"
            or property_name == "r200mean"
            or property_name == "v200mean"
        ):
        ...
        if property_name in catalog_feature_names:
            ...
            prop_vals = self.x_catalog_features[
                :, 0
            ]  # only 1 feature so 2nd dim should be 1
        ...
        for halo in self.dark_halo_arr:
            if "merger" in property_name:
                ...
                property_value = self.subhalos_hydro["SubhaloMassType"][
                    :, self.ipart_star
                ][halo.idx_subhalo_hydro]
                ...
                v_200m = np.sqrt(
                    G
                    * (self.mass_multiplier * halo.catalog_properties["m200m"] * u.Msun)
                    / (halo.catalog_properties["r200m"] * u.kpc)
                )
                ...
                property_value = self.subhalos_hydro["SubhaloPos"][
                    halo.idx_subhalo_hydro
                ]
                ...
                property_value = self.subhalos_hydro["SubhaloSFR"][
                    halo.idx_subhalo_hydro
                ]
            elif property_name == "radius_hydro_subhalo_star":
                property_value = self.subhalos_hydro["SubhaloHalfmassRadType"][
                    :, self.ipart_star
                ][halo.idx_subhalo_hydro]
            elif property_name == "subhalo_hydro_flag":
                property_value = self.subhalos_hydro["SubhaloFlag"][
                    halo.idx_subhalo_hydro
                ]
                ...
                property_value = self.subhalos_hydro["SubhaloVelDisp"][
                    halo.idx_subhalo_hydro
                ]
            elif property_name == "bhmass":
                property_value = self.subhalos_hydro["SubhaloBHMass"][
                    halo.idx_subhalo_hydro
                ]
            elif property_name == "bhmass_per_mstellar":
                m_stellar = self.subhalos_hydro["SubhaloMassType"][:, self.ipart_star][
                    halo.idx_subhalo_hydro
                ]
                ...
                property_value = (
                    10 ** (np.log10(bhmass) - np.log10(m_stellar)) if bhmass != 0 else 0
                )
                ...
                property_value = (
                    phot[halo.idx_subhalo_hydro, 1, 0]
                    - phot[halo.idx_subhalo_hydro, 3, 0]
                )
                ...
                property_value = self.subhalos_dark["SubhaloVelDisp"][
                    halo.idx_subhalo_dark
                ]
            elif property_name == "spin_dm":
                spin_x, spin_y, spin_z = self.subhalos_dark["SubhaloSpin"][
                    halo.idx_subhalo_dark
                ]
    ...
    def get_halos_with_structure_info(self, idxs_halo_dark):
        ...
        print(
            f"{np.sum(i_has_halo_structure_info)}/{len(i_has_halo_structure_info)} halos have halo structure info"
        )
    ...
    def get_structure_catalog_features(self, catalog_feature_names):
        ...
        with h5py.File(
            ...
            for i, c_feat in enumerate(catalog_feature_names):
                x_catalog_features_all.append(f[c_feat][:])
        x_catalog_features_all = np.array(x_catalog_features_all).T
        idxs_halos_dark = np.array(
            [dark_halo.idx_halo_dark for dark_halo in self.dark_halo_arr]
        )
        self.x_catalog_features = x_catalog_features_all[idxs_halos_dark]
        idxs_nan_structure_catalog = np.argwhere(
            np.isnan(self.x_catalog_features).any(axis=1)
        ).flatten()
        assert (
            len(idxs_nan_structure_catalog) == 0
        ), "Halos with NaN values of structure properties detected!"
    ...
    def get_Mvir_SAM(self):
        ...
        halo_idx_to_Mvir_dict = dict(
            zip(halos_sam["HalopropFoFIndex_DM"], halos_sam["HalopropMvir"])
        )
        mvirs = [
            (
                halo_idx_to_Mvir_dict[halo.idx_halo_dark]
                if halo.idx_halo_dark in halo_idx_to_Mvir_dict
                else -1
            )
            for halo in self.dark_halo_arr
        ]
    ...
    def add_MAH_to_halos_SAM(self, halo_tag, most_massive=True):
        ...
        for halo in self.dark_halo_arr:
            ...
            mtree = ilsam.merger.load_tree_haloprop(
                self.base_path_sam,
                root_idx,
                fields=["HalopropRedshift", "HalopropMvir"],
                most_massive=most_massive,
                matches=True,
            )
            scale_factors = 1 / (1 + mtree[root_idx]["HalopropRedshift"])
            halo.set_catalog_property(
                "MAH", [scale_factors, mtree[root_idx]["HalopropMvir"]]
            )
    ...
    def add_MAH_to_halos_sublink(self):
        ...
        if os.path.exists(fn_snaps_redshifts):
            snap_num_list, redshift_list = np.loadtxt(
                fn_snaps_redshifts, usecols=(0, 1), unpack=True
            )
            snap_to_redshift_dict = dict(zip(snap_num_list, redshift_list))
        else:
            raise ValueError(
                f"Snap num redshift file {fn_snaps_redshifts} doesn't exist!"
            )

        fields = ["SubhaloMass", "SubfindID", "SnapNum"]
        count = 0
        for halo in self.dark_halo_arr:
            mtree = il.sublink.loadTree(
                self.base_path_dark,
                self.snap_num,
                halo.idx_subhalo_dark,
                fields=fields,
            ...
            redshifts = np.array(
                [snap_to_redshift_dict[i_snap] for i_snap in mtree["SnapNum"]]
            )
            ...
            halo.set_catalog_property(
                property_name, [scale_factors, mtree["SubhaloMass"]]
            )
            count += 1
    ...
    def add_merger_info_to_halos_sublink(self, halo_tag):
        ...
        for halo in self.dark_halo_arr:
            tree = il.sublink.loadTree(
                self.base_path_dark, self.snap_num, halo.idx_subhalo_dark, fields=fields
            )
            ...
            numMajorMergers = utils.num_mergers_mpb(
                tree, minMassRatio=ratio, massPartType="dm"
            )
            ratioLastMajorMerger = utils.last_merger_ratio_mpb(
                tree, minMassRatio=ratio, massPartType="dm"
            )
            if count % 1000 == 0:
                print(count)
    ...
    def get_mean_density_from_mr200m(self):
        ...
        for halo in self.dark_halo_arr:
            ...
            mean_densities.append(
                mean_dens
            )  # mean of the calculated mean densities for all halos
