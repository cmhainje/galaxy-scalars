...
class GeometricFeaturizer:
    ...
    def featurize(
        ...
        for i, idx_halo_dark in enumerate(self.idxs_halos_dark):
            halo = DarkHalo(
                idx_halo_dark,
                sim_reader.base_path_dark,
                sim_reader.snap_num,
                sim_reader.box_size,
            )

            x_halo, v_halo = halo.load_positions_and_velocities(
                shift=True, pos_center=tab_halos["x_minPE"][i]
            )

            r_edges_scaled = r_edges
            if r_units is not None:
                r_edges_scaled = r_edges * tab_halos[r_units][i]
            self.geo_feature_arr.append(
                self.get_geometric_features(
                    x_halo,
                    v_halo,
                    r_edges_scaled,
                    l_arr,
                    p_arr,
                    self.sim_reader.m_dmpart_dark,
                )
            )

    def get_geometric_features(
        self, delta_x_data_halo, delta_v_data_halo, r_edges, l_arr, p_arr, m_dm, n_dim=3
    ):

        # input checks
        N, n_dim_from_data = delta_x_data_halo.shape
        assert (
            n_dim_from_data == n_dim
        ), f"Number of dimensions in data {n_dim_from_data} does not equal global n_dim, {n_dim}!"
        assert (
            delta_x_data_halo.shape == delta_v_data_halo.shape
        ), "Position and velocity arrays should have same shape!"
        ...
        for i_l, l in enumerate(l_arr):
            for i_p, p in enumerate(p_arr):
                ...
                for i_n, n in enumerate(n_arr):
                    ...
                    if l == 1 and p == 1:
                        hermitian = False
                        xv_terms = vector_outer_product(
                            x_outers_arr[i_l], v_outers_arr[i_p]
                        )
                    else:
                        hermitian = True
                        xv_terms = multiply_vector_terms(
                            x_outers_arr[i_l], v_outers_arr[i_p]
                        )

                    g_lpn = (
                        np.sum(
                            multiply_vector_terms(window_vals[i_n], xv_terms), axis=0
                        )
                        * m_dm
                    )

                    geo = GeometricFeature(
                        g_lpn, m_order=1, x_order=l, v_order=p, n=n, hermitian=hermitian
                    )
...
class GeometricFeature:
    ...
    def __init__(
        self, value, m_order, x_order, v_order, n, hermitian=True, modification=None
    ):
        self.value = value
...
def geo_name(geometric_feature, mode="readable"):
    ...
    if mode == "multipole":
        ...
        if (
            geometric_feature.modification is not None
            and geometric_feature.modification != "None"
        ):
...
def rebin_geometric_features(geo_feature_arr, n_groups):
    ...
    n_vals = [
        g.n for g in geo_feature_arr[0]
    ]  # 0 because just check first halo, features should be same
    n_groups_flat = [n for group in n_groups for n in group]
    assert set(n_groups_flat).issubset(
        set(n_vals)
    ), "Groups passed in contain bins not in geometric features!"
        ...
        for i_newn, geo_feat_group in enumerate(geo_feats_grouped):
            ...
            for x_order in range(x_order_highest + 1):
                for v_order in range(v_order_highest + 1):
                    geo_feats_this_order = [
                        g
                        for g in geo_feat_group
                        if g.x_order == x_order and g.v_order == v_order
                    ]
                    # continue if there are no values at this order (e.g. none at x=2, v=1)
                    if not geo_feats_this_order:
                        continue
                    geo_rebinned_value = np.sum(
                        [g.value for g in geo_feats_this_order], axis=0
                    )
                    hermitian = geo_feats_this_order[
                        0
                    ].hermitian  # if one is hermitian, all are at this order
                    geo_rebinned = GeometricFeature(
                        geo_rebinned_value,
                        m_order=1,
                        x_order=x_order,
                        v_order=v_order,
...
def transform_pseudotensors(geo_feature_arr):
    ...
    for i_halo, geo_features_halo in enumerate(geo_feature_arr):
        ...
        for i_feat, g in enumerate(geo_features_halo):
            ...
            if not g.hermitian and g.modification is None:
                ...
                g_symm = GeometricFeature(
                    ...
                    n=g.n,
                    hermitian=True,
                    modification="symmetrized",
                )
                g_antisymm = GeometricFeature(
                    g_value_antisymm,
                    m_order=g.m_order,
                    x_order=g.x_order,
                    v_order=g.v_order,
        ...
        for idxs_to_insert, g_to_insert in zip(idxs_to_insert, gs_to_insert):
            geo_feature_arr[i_halo] = np.insert(
                geo_feature_arr[i_halo], idxs_to_insert, g_to_insert
            )
