...
label_dict = {
    "log_m200m_fof": r"log($M_\mathrm{200m,FoF} \: [h^{-1} \, M_\odot]$)",
...
def get_uncertainties_genel2019(gal_property, log_m_stellar, sim_name):
    ...
    assert (
        gal_property in gal_properties
    ), f"Property {gal_property} not in gal_properties={gal_properties}"
    ...
    assert np.min(log_m_stellar) > np.min(logmstellar_bins) and np.max(
        log_m_stellar
    ) < np.max(logmstellar_bins), err_msg

    # added estimates on either end (low: double, high: extend)
    stdev_dict_mstellar = {
        "TNG50-4": np.array([0.56, 0.28, 0.23, 0.12, 0.05, 0.04, 0.04]),
        "TNG100-1": np.array([0.16, 0.08, 0.06, 0.04, 0.03, 0.04, 0.04]),
    }
    stdev_dict_rstellar = {
        "TNG50-4": np.array([0.42, 0.21, 0.14, 0.09, 0.07, 0.06, 0.06]),
        "TNG100-1": np.array([0.14, 0.07, 0.08, 0.08, 0.09, 0.08, 0.08]),
    }

    stdev_dict_sfr = {
        "TNG50-4": np.array([0.72, 0.36, 0.34, 0.28, 0.30, 0.45, 0.45]),
        "TNG100-1": np.array([0.44, 0.22, 0.21, 0.22, 0.34, 0.62, 0.62]),
    }

    stdev_dict_sfr1 = {
        "TNG50-4": np.array([0.82, 0.41, 0.28, 0.25, 0.25, 0.4, 0.4]),
        "TNG100-1": np.array([0.34, 0.17, 0.15, 0.16, 0.25, 0.5, 0.5]),
    }

    stdev_dict_bhmass = {
        "TNG50-4": np.array([0.26, 0.13, 0.12, 0.11, 0.08, 0.07, 0.07]),
        "TNG100-1": np.array([0.22, 0.11, 0.13, 0.18, 0.12, 0.06, 0.06]),
    }

    gal_property_to_stdev_dict = {
        "m_stellar": stdev_dict_mstellar,
...
def broken_power_law(log_m200, N=1, log_m1=12, beta=1, gamma=1):
    ...
    return log_m200 + np.log10(
        2 * N / ((log_m200 / log_m1) ** (-beta) + (log_m200 / log_m1) ** gamma)
    )
...
def fit_function(function, x, y, uncertainties, params_initial_guess):
    ...
    params_best_fit, _ = curve_fit(
        function, x, y, sigma=uncertainties, p0=params_initial_guess
    )
...
def split_train_val_test(random_ints, frac_train=0.70, frac_val=0.15, frac_test=0.15):
    ...
    tol = 1e-6
    assert (
        abs((frac_train + frac_val + frac_test) - 1.0) < tol
...
label_to_target_name = {
    "r_stellar": "radius_hydro_subhalo_star",
...
def get_y_vals(y_label_name, sim_reader, mass_multiplier=1e10, halo_tag=""):
    ...
    y_target_name = (
        label_to_target_name[y_label_name]
        if y_label_name in label_to_target_name
        else y_label_name
    )
    sim_reader.add_catalog_property_to_halos(y_target_name, halo_tag=halo_tag)
    y_vals = np.array(
        [halo.catalog_properties[y_target_name] for halo in sim_reader.dark_halo_arr]
    )

    if (
        y_label_name == "m_stellar"
        or y_label_name == "r_stellar"
        or y_label_name == "num_mergers"
        or y_label_name == "j_stellar"
    ):
        ...
        m_stellar = np.array(
            [
                halo.catalog_properties["mass_hydro_subhalo_star"]
        ...
        log_ssfr = log_sfr_to_log_ssfr(
            log_sfr, m_stellar, mass_multiplier=mass_multiplier
        )
        return log_ssfr
        ...
        val_zerobhms = (
            np.min(bhmass_per_mstellar[~i_zerobhms]) / 10.0
        )  # set zero value to 1/10 of nonzero-min
...
def get_y_uncertainties(
    y_label_name, sim_reader=None, y_vals=None, log_mass_shift=10, idx_train=None
):

    ssfr_name_to_sfr_name = {"ssfr": "sfr", "ssfr1": "sfr1"}
    if y_label_name == "ssfr" or y_label_name == "ssfr1":
        assert (
            sim_reader is not None and y_vals is not None
        ), "Must pass sim_reader and y_vals!"
        ...
        uncertainties_genel2019_sfr = get_uncertainties_genel2019(
            sfr_label_name_genel2019,
            np.log10(m_stellar) + log_mass_shift,
        ...
        uncertainties_genel2019_poisson_sfr = np.sqrt(
            uncertainties_genel2019_sfr**2 + uncertainties_poisson_sfr**2
        )
        uncertainties_genel2019_poisson_ssfr = (
            uncertainty_log_sfr_to_uncertainty_log_ssfr(
        ...
        y_uncertainties = get_uncertainties_genel2019(
            y_label_name,
            np.log10(m_stellar) + log_mass_shift,
            sim_name=sim_reader.sim_name,
        )
        ...
        y_uncertainties_bhmass = get_uncertainties_genel2019(
            "bhmass", np.log10(m_stellar) + log_mass_shift, sim_name=sim_reader.sim_name
        )
        y_uncertainties_mstellar = get_uncertainties_genel2019(
            "m_stellar",
            np.log10(m_stellar) + log_mass_shift,
            sim_name=sim_reader.sim_name,
        )
        ...
        y_uncertainties = np.sqrt(
            (y_uncertainties_bhmass) ** 2 + (y_uncertainties_mstellar) ** 2
        )
        ...
        assert (
            idx_train is not None
        ), "Must pass idx_train to get uncertainty for Mofa or a_mfrac!"
...
def get_mrv_for_rescaling(tab_halos, fn_geo_clean_config, use_logs=True):
    # Get MRV rescaling
...
def load_features(
    ...
    assert feature_mode in [
        ...
        "catalogz0",
        "mrv",
    ], f"Feature mode {feature_mode} not recognized!"
    ...
    if feature_mode == "scalars":
        ...
        x_extra = get_mrv_for_rescaling(tab_halos, fn_geo_clean_config, use_logs=True)
...
def load_labels(label_names, tab_halos, tab_select, fn_amfrac=None):
...
def load_uncertainties(label_names, fn_unc, tab_select):
    idxs_table = tab_select["idx_table"]
...
def get_butterfly_error(
    x_bins, y_label_name, halo_logmass_min=None, x_label_name="log_m200m"
):
    arr_shadow1 = np.loadtxt(
        "../data/butterfly_L25n512TNGs35_shadow1.csv", skiprows=1, delimiter=","
    )
    arr_shadow2 = np.loadtxt(
        "../data/butterfly_L25n512TNGs35_shadow2.csv", skiprows=1, delimiter=","
    )
    ...
    if y_label_name == "log_ssfr":
        ...
        val_zero = np.log10(
            zero_dict["mbh"]
        )  # this is for log_mbh not per mstellar! and in real not code units
