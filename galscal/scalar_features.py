...
from . import utils
from . import geometric_features as gf
from .geometric_features import geo_name
...
class ScalarFeaturizer:
    ...
    def featurize(
        ...
        for geo_features_halo in self.geo_feature_arr:
            scalar_arr_i = self.get_scalar_features(
                geo_features_halo,
                m_order_max,
                x_order_max,
                v_order_max,
    ...
    def get_scalar_features(
        ...
        assert (
            m_order_max >= 1 and m_order_max <= 2
        ), "m_order_max must be 1 or 2 (for now!)"
                    ...
                    # 2 tensors or 2 pseudotensors: full contraction
                        ...
                        if g1.hermitian == g2.hermitian:
                            if not g1.hermitian or not g2.hermitian:
                                ...
                                assert (
                                    g1.modification == "antisymmetrized"
                                    and g2.modification == "antisymmetrized"
                                ), msg
    ...
    def sort_scalar_features(self, scalar_features, scalar_feature_arr):
        scalar_features_single = scalar_feature_arr[
            0
        ]  # list of features for single halo
        ...
        scalar_feature_table["name"] = [
            scalar_name(s, mode="readable") for s in scalar_features_single
        ]
        ...
        for i in range(self.m_order_max):
            ...
            scalar_feature_table[f"n_{i}"] = [
                s.ns[i] if len(s.ns) > i else -1 for s in scalar_features_single
            ]
    ...
    def compute_M_tot(self, geo_feature_arr_onebin):
        ...
        for i_g, geo_features_halo in enumerate(self.geo_feature_arr):
            self.M_tot[i_g] = np.sum(
                [
                    g.value
                    for g in geo_feature_arr_onebin[i_g]
        ...
        for i_g, geo_features_halo in enumerate(self.geo_feature_arr):
            x_tensors = np.array(
                [
                    g.value
                    for g in geo_feature_arr_onebin[i_g]
                    if g.x_order == 2 and g.v_order == 0
                ]
            )
            assert (
                len(x_tensors) == 1
            ), "Should be exactly one second order tensor for x!"
        ...
        for i_g, geo_features_halo in enumerate(self.geo_feature_arr):
            v_tensors = np.array(
                [
                    g.value
                    for g in geo_feature_arr_onebin[i_g]
                    if g.x_order == 0 and g.v_order == 2
                ]
            )
            assert (
                len(v_tensors) == 1
            ), "Should be exactly one second order tensor for v!"
    ...
    def save_features(self, fn_scalar_features, save_format="table", overwrite=True):
        if save_format == "table":
            tab_scalars = scalar_objects_to_table(
                self.scalar_feature_arr, self.idxs_halos_dark
            )
...
def scalar_name(scalar_feature, mode="readable"):
    ...
    for i, g_name in enumerate(g_names):
        if scalar_feature.operations[i] == "" or scalar_feature.operations[i] is None:
    ...
    for i in range(len(tab_scalars)):
        ...
        for j in range(len(tab_scalar_info)):
            ...
            scalar = ScalarFeature(
                value=tab_scalars[i][scalar_key],
                geo_keys=geo_keys,
                geo_names=geo_names,
